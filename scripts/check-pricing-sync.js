...
const samples = [
  { p: 'deepseek-official', m: 'deepseek-v4.1-flash', ts: Date.UTC(2026, 8, 10, 5, 0), t: { input: 3850, output: 2880, cacheRead: 43904, cacheWrite: 0, reasoning: 0 } },
...
for (const s of samples) {
  ...
  if (pc.model !== cc.model || pc.tiered !== cc.tiered || pc.subscription !== cc.subscription) {
    bad += 1
    console.error('✗ 解析不一致: ' + s.m + ' 插件=' + JSON.stringify({ m: pc.model, t: pc.tiered }) + ' 云端=' + JSON.stringify({ m: cc.model, t: cc.tiered }))
    continue
  }
